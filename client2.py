...
from pymodbus.client import ModbusTcpClient, ModbusBaseClient
...
def create_sunspec_sync_client(host, port):
    ...
    """
    modbus = ModbusTcpClient(host, port=port)
    modbus.connect()
...
def __init__(self, payload, byteorder=Endian.LITTLE, wordorder=Endian.BIG):
    def __init__(self, payload, byteorder, **kwargs):
        ...
        BinaryPayloadDecoder.__init__(self, payload, byteorder, **kwargs)
...
class SunspecClient:
    ...
    def get_device_block(self, offset, size=1):
        ...
        response = self.client.read_holding_registers(offset, count=size + 2)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the SunSpec client.")
    parser.add_argument("--ip", required=True, help="The IP address of the SunSpec device.")
    parser.add_argument("--port", type=int, default=502, help="The port of the SunSpec device (default: 502).")
    args = parser.parse_args()

    client = create_sunspec_sync_client(args.ip, args.port)

    # print out all the device common block
    common = client.get_common_block()
    for key, value in common.items():  # Changed iteritems() to items() for Python 3 compatibility
        if key == "SunSpec_DID":
